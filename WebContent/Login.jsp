...
<body class="home">
   <div id="event" style="margin-bottom:-1vw; width:auto">
    ...
    <% 		
		...
		if(msg == null) {
			msg="";
		}else if(msg.equals("pwdError"))
		{
			out.println("<script>alert('패스워드 오류!!!');</script>");
		}
		else if(msg.equals("There is no id in DB"))
		{
			out.println("<script>alert('아이디가 존재하지 않습니다!!');</script>");
		}
		else if(msg.equals("(ADMIN)pwdError"))
		{
			out.println("<script>alert('(어드민)패스워드 오류!!');</script>");
		}
...
<body class="align" >
  ...
  <div class="site__container" style="border:2px solid #6D6A6A; width:36vw; margin-left:30vw; height:35vw">

    <div class="grid__container">
      <div style="color:black; font-size:3vw; text-align:center">Login</div>
      <form action="memberLogin.do" method="post" class="form form--login" style="margin-top:3.3vw; width:26vw; margin-left:-5.5vw">

        <div class="form__field">
          <label class="fontawesome-user fa fa-user fa-2x" for="login__username" style=""><span class="hidden">Userid</span></label>
          <input name="id" type="text" style="width:8vw"class="form__input" placeholder="Username" required>
        </div>

        <div class="form__field">
          <label class="fontawesome-lock fa fa-lock fa-2x" for="login__password"><span class="hidden">Password</span></label>
          <input name="passwd" type="password" class="form__input" placeholder="Password" required>
        </div>

        <div class="form__field" >
          <input type="submit" style="height:5vw; font-size:2vw" value="Log In" >
           <input type="hidden" name="job" value="login" />
        </div>
 
      </form>
	<h3 style="font-size:1vw; color:red; width:30vw; margin-left:3vw">${error}</h3>
      <p class="text--center;" style="width:20vw; font-size:1vw">Not a member? <a href="Register.jsp" style="font-size:1vw">Sign up now</a> <span class="fontawesome-arrow-right"></span></p>
